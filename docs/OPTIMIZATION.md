...
**⚠️ Важно:** этот документ — **справочник** на весь жизненный цикл продукта. Он описывает, **что и когда** оптимизировать. На MVP (Фаза 1) большинство этих оптимизаций **не нужны**. Применяй их по мере реальной потребности, ориентируясь на `ROADMAP.md`.

**Правило:** не оптимизируй, пока не измерил. Не добавляй кэш, пока не увидел повторный запрос. Не добавляй SSE, пока пользователи не попросили.
