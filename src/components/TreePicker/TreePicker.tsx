...
export function TreePicker({
  ...
  const renderCell = (domain: ToolDomain) => {
    const allSubs = getSubsByDomain(domain);
    const subs = filterSet ? allSubs.filter(({ sub }) => filterSet.has(sub.id)) : allSubs;
      ...
      >
        ...
        {subs.length > 0 && (
          <div className={styles.cellBody}>
            {(() => {
              let lastCatId = '';
              return subs.map(({ catId, catName, sub }) => {
                ...
                return (
                  <div key={sub.id}>
                    {showHeader && subs.length > 1 && (
                      <div className={styles.cellCatHeader}>{catName}</div>
                    )}
                    <button
  ...
  return (
    <div className={`${styles.wrapper} ${fullHeight ? styles.wrapperFull : ''}`}>
      ...
      {sidebarFooter && <div className={styles.footer}>{sidebarFooter}</div>}
    </div>
  );
}
